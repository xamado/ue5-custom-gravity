...
DECLARE_DELEGATE_RetVal_ThreeParams(FTransform, FOnProcessRootMotion, const FTransform&, UBaseCharacterMovementComponent*, float)
...
/** 
 * Tick function that calls UBaseCharacterMovementComponent::PostPhysicsTickComponent
 **/
USTRUCT()
struct FBaseCharacterMovementComponentPostPhysicsTickFunction : public FTickFunction
{
	GENERATED_USTRUCT_BODY()

	/** CharacterMovementComponent that is the target of this tick **/
	class UBaseCharacterMovementComponent* Target;
...
template<>
struct TStructOpsTypeTraits<FBaseCharacterMovementComponentPostPhysicsTickFunction> : public TStructOpsTypeTraitsBase2<FBaseCharacterMovementComponentPostPhysicsTickFunction>
{
	enum
	{
		WithCopy = false
...
USTRUCT()
struct FBaseCharacterMovementComponentPrePhysicsTickFunction : public FTickFunction
{
	GENERATED_USTRUCT_BODY()
...
template<>
struct TStructOpsTypeTraits<FBaseCharacterMovementComponentPrePhysicsTickFunction> : public TStructOpsTypeTraitsBase2<FBaseCharacterMovementComponentPrePhysicsTickFunction>
{
	enum
	{
		WithCopy = false
 ...
 *
 * @see ABaseCharacter, UPawnMovementComponent
 * @see https://docs.unrealengine.com/latest/INT/Gameplay/Framework/Pawn/Character/
 */

UCLASS(MinimalAPI)
class UBaseCharacterMovementComponent : public UPawnMovementComponent, public IRVOAvoidanceInterface, public INetworkPredictionInterface
{
	GENERATED_BODY()
public:
...
public:
	...
	*/
	 ...
	 */
	virtual bool ShouldCatchAir(const FBaseFindFloorResult& OldFloor, const FBaseFindFloorResult& NewFloor);
	 ...
	 */
	virtual bool StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult &Hit, FBaseStepDownResult* OutStepDownResult = NULL);
	 ...
	 */
	void SetBaseFromFloor(const FBaseFindFloorResult& FloorResult);
	...
	/** Whether the gravity direction is different from UBaseCharacterMovementComponent::DefaultGravityDirection. */
...
public:
	...
	virtual bool CheckFall(const FBaseFindFloorResult& OldFloor, const FHitResult& Hit, const FVector& Delta, const FVector& OldLocation, float remainingTime, float timeTick, int32 Iterations, bool bMustJump);
	
	/** 
	 *  Revert to previous position OldLocation, return to being based on OldBase.
	 *  if bFailMove, stop movement and notify controller
	 */	
	void RevertMove(const FVector& OldLocation, UPrimitiveComponent* OldBase, const FVector& InOldBaseLocation, const FBaseFindFloorResult& OldFloor, bool bFailMove);
	 ...
	 */
	virtual void MoveSmooth(const FVector& InVelocity, const float DeltaSeconds, FBaseStepDownResult* OutStepDownResult = NULL );
	...
	struct FBaseCharacterMovementComponentPrePhysicsTickFunction PrePhysicsTickFunction;

	/** Tick function called before physics */
	virtual void PrePhysicsTickComponent(float DeltaTime, FBaseCharacterMovementComponentPrePhysicsTickFunction& ThisTickFunction);

	/** Post-physics tick function for this character */
	UPROPERTY()
	struct FBaseCharacterMovementComponentPostPhysicsTickFunction PostPhysicsTickFunction;

	/** Tick function called after physics (sync scene) has finished simulation, before cloth */
	virtual void PostPhysicsTickComponent(float DeltaTime, FBaseCharacterMovementComponentPostPhysicsTickFunction& ThisTickFunction);
...
protected:
	 ...
	 * @param OutStepDownResult:	[Out] If non-null, and a floor check is performed, this will be updated to reflect that result.
	 */
	virtual void MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FBaseStepDownResult* OutStepDownResult = NULL);
...
public:
	 ...
	 */
	virtual void FindFloor(const FVector& CapsuleLocation, FBaseFindFloorResult& OutFloorResult, bool bCanUseCachedLocation, const FHitResult* DownwardSweepResult = NULL) const;
	...
	*/
	...
	virtual void K2_FindFloor(FVector CapsuleLocation, FBaseFindFloorResult& FloorResult) const;
	 ...
	 * @param DownwardSweepResult:	If non-null and it contains valid blocking hit info, this will be used as the result of a downward sweep test instead of doing it as part of the update.
	 */
	virtual void ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FBaseFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult = NULL) const;
	...
	*/
	...
	virtual void K2_ComputeFloorDist(FVector CapsuleLocation, float LineDistance, float SweepDistance, float SweepRadius, FBaseFindFloorResult& FloorResult) const;
	 ...
	 */
	virtual bool ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FBaseFindFloorResult& OutPerchFloorResult) const;
...
protected:
	 ...
	 */
	FVector GetPawnCapsuleExtent(const EBaseShrinkCapsuleExtent ShrinkMode, const float CustomShrinkAmount = 0.f) const;
	 ...
	 * @return The capsule extent of the Pawn owner, possibly reduced in size depending on ShrinkMode.
	 */
	FCollisionShape GetPawnCapsuleCollisionShape(const EBaseShrinkCapsuleExtent ShrinkMode, const float CustomShrinkAmount = 0.f) const;
...
protected:
	...
	/** Called by UBaseCharacterMovementComponent::VerifyClientTimeStamp() when a client timestamp reset has been detected and is valid. */
	 ...
	 * Called by UBaseCharacterMovementComponent::VerifyClientTimeStamp() for valid timestamps.
	 */
	...
	/** 
	 * Called by UBaseCharacterMovementComponent::ProcessClientTimeStampForTimeDiscrepancy() (on server) when the time from client moves 
...
public:
	...
	/**
	 * The actual network RPCs for character movement are passed to ABaseCharacter, which wrap to the _Implementation call here, to avoid Component RPC overhead.
	 * For example:
	 *		Client: UBaseCharacterMovementComponent::ServerMovePacked_ClientSend() => Calls CharacterOwner->ServerMove() triggering RPC on the server.
	 *		Server: ABaseCharacter::ServerMovePacked_Implementation() from the RPC => Calls CharacterMovement->ServerMove_ServerReceive(), unpacked and sent to ServerMove_ServerHandleMoveData().
	 *
	 *	ServerMove_ClientSend() and ServerMove_ServerReceive() use a bitstream created from the current FBaseCharacterNetworkMoveData data container that contains the client move.
	 *	See GetNetworkMoveDataContainer()/SetNetworkMoveDataContainer() for details on setting a custom container with custom unpacking through FBaseCharacterNetworkMoveData::Serialize().
	 * 
	*/
	 ...
	 */
	void ServerMovePacked_ClientSend(const FBaseCharacterServerMovePackedBits& PackedBits);

	/**
	 * On the server, receives packed move data from the Character RPC, unpacks them into the FBaseCharacterNetworkMoveDataContainer returned from GetNetworkMoveDataContainer(),
	...
	void MoveResponsePacked_ClientReceive(const FBaseCharacterMoveResponsePackedBits& PackedBits);
	...
	virtual void ClientAdjustRootMotionSourcePosition_Implementation(float TimeStamp, FBaseRootMotionSourceGroup ServerRootMotion, bool bHasAnimRootMotion, float ServerMontageTrackPosition, FVector ServerLoc, FVector_NetQuantizeNormal ServerRotation, float ServerVelZ, UPrimitiveComponent* ServerBase, FName ServerBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode);
	 ...
	 */
	virtual void ServerMove_HandleMoveData(const FBaseCharacterNetworkMoveDataContainer& MoveDataContainer);
	 ...
	 * The FBaseCharacterNetworkMoveData parameter to this function is also the same returned by GetCurrentNetworkMoveData(), to assist in migration of code that may want to access the data without changing function signatures.
	 * (Note: this is similar to "ServerMove_Implementation" in legacy versions).
	 */
	virtual void ServerMove_PerformMovement(const FBaseCharacterNetworkMoveData& MoveData);

	/**
	 * On the server, sends a packed move response to the client. First the FBaseCharacterMoveResponseDataContainer from GetMoveResponseDataContainer() is filled in with ServerFillResponseData().
	 * Then this data is serialized to a bit stream that is sent to the client via MoveResponsePacked_ServerSend().
	 */
	void ServerSendMoveResponse(const FBaseClientAdjustment& PendingAdjustment);
...
protected:
	 ...
	 */
	void SetNetworkMoveDataContainer(FBaseCharacterNetworkMoveDataContainer& PersistentDataStorage) { NetworkMoveDataContainerPtr = &PersistentDataStorage; }

	/**
	 * Get the struct used for client to server move RPC serialization.
	 * @see SetNetworkMoveDataContainer()
	 */
	FBaseCharacterNetworkMoveDataContainer& GetNetworkMoveDataContainer() const { return *NetworkMoveDataContainerPtr; }
	 ...
	 */
	FBaseCharacterNetworkMoveData* GetCurrentNetworkMoveData() const { return CurrentNetworkMoveData; }

	/**
	 * Used internally to set the FBaseCharacterNetworkMoveData currently being processed, either being serialized or replayed on the client, or being received and processed on the server.
	 * @see GetCurrentNetworkMoveData()
	 */
	void SetCurrentNetworkMoveData(FBaseCharacterNetworkMoveData* CurrentData) { CurrentNetworkMoveData = CurrentData; }
	 ...
	 */
	void SetMoveResponseDataContainer(FBaseCharacterMoveResponseDataContainer& PersistentDataStorage) { MoveResponseDataContainerPtr = &PersistentDataStorage; }
	 ...
	 */
	FBaseCharacterMoveResponseDataContainer& GetMoveResponseDataContainer() const { return *MoveResponseDataContainerPtr; }
...
private:
	...
	FBaseCharacterNetworkMoveDataContainer DefaultNetworkMoveDataContainer;

	/** Pointer to server move RPC data container. */
	FBaseCharacterNetworkMoveDataContainer* NetworkMoveDataContainerPtr;
	...
	FBaseCharacterNetworkMoveData* CurrentNetworkMoveData;

	//////////////////////////////////////////////////////////////////////////
	// Server response data

	/** Default server response RPC data container. Can be bypassed via SetMoveResponseDataContainer(). */
	FBaseCharacterMoveResponseDataContainer DefaultMoveResponseDataContainer;

	/** Pointer to server response RPC data container. */
	FBaseCharacterMoveResponseDataContainer* MoveResponseDataContainerPtr;
...
public:
	...
	UPROPERTY(Transient)
	FBaseRootMotionSourceGroup CurrentRootMotion;
	...
	uint16 ApplyRootMotionSource(TSharedPtr<FBaseRootMotionSource> SourcePtr);
	...
	virtual void OnRootMotionSourceBeingApplied(const FBaseRootMotionSource* Source);

	/** Get a RootMotionSource from current root motion by name */
	TSharedPtr<FBaseRootMotionSource> GetRootMotionSource(FName InstanceName);

	/** Get a RootMotionSource from current root motion by ID */
	TSharedPtr<FBaseRootMotionSource> GetRootMotionSourceByID(uint16 RootMotionSourceID);
	...
	void ConvertRootMotionServerIDsToLocalIDs(const FBaseRootMotionSourceGroup& LocalRootMotionToMatchWith, FBaseRootMotionSourceGroup& InOutServerRootMotion, float TimeStamp);

	/** Collection of the most recent ID mappings */
	enum class ERootMotionMapping : uint32 { MapSize = 16 };
	TArray<FBaseRootMotionServerToLocalIDMapping, TInlineAllocator<(uint32)ERootMotionMapping::MapSize> > RootMotionIDMappings;
...
FORCEINLINE ABaseCharacter* UBaseCharacterMovementComponent::GetCharacterOwner() const
{
	return CharacterOwner;
}

FORCEINLINE_DEBUGGABLE bool UBaseCharacterMovementComponent::IsWalking() const
{
	return IsMovingOnGround();
}

FORCEINLINE uint32 UBaseCharacterMovementComponent::PackYawAndPitchTo32(const float Yaw, const float Pitch)
{
...
public:
	...
	FBaseFindFloorResult StartFloor;
	...
	FBaseRootMotionSourceGroup SavedRootMotion;
	...
	virtual void SetMoveFor(ABaseCharacter* C, float InDeltaTime, FVector const& NewAccel, class FNetworkPredictionData_Client_Character & ClientData);

	/** Set the properties describing the position, etc. of the moved pawn at the start of the move. */
	virtual void SetInitialPosition(ABaseCharacter* C);
	...
	virtual void PostUpdate(ABaseCharacter* C, EPostUpdateMode PostUpdateMode);
	
	/** Returns true if this move can be combined with NewMove for replication without changing any behavior */
	virtual bool CanCombineWith(const FSavedMovePtr& NewMove, ABaseCharacter* InCharacter, float MaxDelta) const;

	/** Combine this move with an older move and update relevant state. */
	virtual void CombineWith(const FSavedMove_Character* OldMove, ABaseCharacter* InCharacter, APlayerController* PC, const FVector& OldStartLocation);
	
	/** Called before ClientUpdatePosition uses this SavedMove to make a predictive correction	 */
	virtual void PrepMoveFor(ABaseCharacter* C);
...
public:
	...
	FNetworkPredictionData_Client_Character(const UBaseCharacterMovementComponent& ClientMovement);
	...
	/**
	 * Copied value from UBaseCharacterMovementComponent::NetworkMaxSmoothUpdateDistance.
	 * @see UBaseCharacterMovementComponent::NetworkMaxSmoothUpdateDistance
	 */
	float MaxSmoothNetUpdateDist;

	/**
	 * Copied value from UBaseCharacterMovementComponent::NetworkNoSmoothUpdateDistance.
	 * @see UBaseCharacterMovementComponent::NetworkNoSmoothUpdateDistance
	 */
	...
	void AckMove(int32 AckedMoveIndex, UBaseCharacterMovementComponent& CharacterMovementComponent);
	...
	float UpdateTimeStampAndDeltaTime(float DeltaTime, ABaseCharacter & CharacterOwner, class UBaseCharacterMovementComponent & CharacterMovementComponent);
...
public:
	...
	FNetworkPredictionData_Server_Character(const UBaseCharacterMovementComponent& ServerMovement);
	virtual ~FNetworkPredictionData_Server_Character();

	FBaseClientAdjustment PendingAdjustment;
