...
#include "BaseRootMotionSource.generated.h"
...
struct BaseRootMotionSourceDebug
{
	static TAutoConsoleVariable<int32> CVarDebugRootMotionSources;
	static void PrintOnScreen(const ABaseCharacter& InCharacter, const FString& InString);
 ...
 */
UENUM()
enum class EBaseRootMotionAccumulateMode : uint8
{
 ...
 */
UENUM()
enum class EBaseRootMotionSourceStatusFlags : uint8
{
 ...
 */
UENUM()
enum class EBaseRootMotionSourceSettingsFlags : uint8
{
...
enum class EBaseRootMotionSourceID : uint16 { Invalid = 0 };

/** 
 * Structure for mapping RootMotionSource server IDs to those on this client
 */
struct FBaseRootMotionServerToLocalIDMapping
{
 ...
 */
USTRUCT()
struct FBaseRootMotionSourceStatus
{
	...
	void SetFlag(EBaseRootMotionSourceStatusFlags Flag);
	void UnSetFlag(EBaseRootMotionSourceStatusFlags Flag);
	bool HasFlag(EBaseRootMotionSourceStatusFlags Flag) const;
};
 ...
 */
USTRUCT()
struct FBaseRootMotionSourceSettings
{
	GENERATED_USTRUCT_BODY()
	...
	void SetFlag(EBaseRootMotionSourceSettingsFlags Flag);
	void UnSetFlag(EBaseRootMotionSourceSettingsFlags Flag);
	bool HasFlag(EBaseRootMotionSourceSettingsFlags Flag) const;

	// Accumulate settings with one another
	FBaseRootMotionSourceSettings& operator+=(const FBaseRootMotionSourceSettings& Other);
};

UENUM()
enum class EBaseRootMotionFinishVelocityMode : uint8
{
 ...
 */
USTRUCT()
struct FBaseRootMotionFinishVelocitySettings
{
	GENERATED_USTRUCT_BODY()

	// Mode, see EBaseRootMotionFinishVelocityMode
	UPROPERTY()
	EBaseRootMotionFinishVelocityMode Mode;
	...
	FBaseRootMotionFinishVelocitySettings()
		: Mode(EBaseRootMotionFinishVelocityMode::MaintainLastRootMotionVelocity)
...
struct FBaseRootMotionSource
{
	...
	/** ID local to this client or server instance. Used for matching up FBaseRootMotionSources between SavedMoves
	 ...
	 **/
	UPROPERTY()
	EBaseRootMotionAccumulateMode AccumulateMode;
	...
	UPROPERTY()
	FBaseRootMotionSourceStatus Status;

	/** Settings of this source */
	UPROPERTY(NotReplicated)
	FBaseRootMotionSourceSettings Settings;
	...
	UPROPERTY(NotReplicated)
	FBaseRootMotionFinishVelocitySettings FinishVelocityParams;

	FBaseRootMotionSource();

	virtual ~FBaseRootMotionSource() {}
	...
	/** @return newly allocated copy of this FBaseRootMotionSource. Must be overridden by child classes. */
	virtual FBaseRootMotionSource* Clone() const;
	 ...
	 *  
	 *  For well-networked RootMotionSources, any given FBaseRootMotionSource child class could implement
	 ...
	 *
	 *  See UBaseCharacterMovementComponent::ConvertRootMotionServerIDsToLocalIDs
	 *
	 *  Should be overridden by child classes, as default implementation only contains basic equivalency checks 
	 **/
	virtual bool Matches(const FBaseRootMotionSource* Other) const;

	/** Checks that it Matches() and has the same state (time, track position, etc.) */
	virtual bool MatchesAndHasSameState(const FBaseRootMotionSource* Other) const;
	 ...
	 **/
	virtual bool UpdateStateFrom(const FBaseRootMotionSource* SourceToTakeStateFrom, bool bMarkForSimulatedCatchup = false);
...
template<>
struct TStructOpsTypeTraits< FBaseRootMotionSource > : public TStructOpsTypeTraitsBase2< FBaseRootMotionSource >
{
	enum
	{
...
USTRUCT()
struct FBaseRootMotionSource_ConstantForce : public FBaseRootMotionSource
{
	GENERATED_USTRUCT_BODY()

	FBaseRootMotionSource_ConstantForce();

	virtual ~FBaseRootMotionSource_ConstantForce() {}
	...
	virtual FBaseRootMotionSource* Clone() const override;

	virtual bool Matches(const FBaseRootMotionSource* Other) const override;

	virtual bool MatchesAndHasSameState(const FBaseRootMotionSource* Other) const override;

	virtual bool UpdateStateFrom(const FBaseRootMotionSource* SourceToTakeStateFrom, bool bMarkForSimulatedCatchup = false) override;
...
template<>
struct TStructOpsTypeTraits< FBaseRootMotionSource_ConstantForce > : public TStructOpsTypeTraitsBase2< FBaseRootMotionSource_ConstantForce >
{
	enum
	{
		WithNetSerializer = true,
...
USTRUCT()
struct FBaseRootMotionSource_RadialForce : public FBaseRootMotionSource
{
	GENERATED_USTRUCT_BODY()

	FBaseRootMotionSource_RadialForce();

	virtual ~FBaseRootMotionSource_RadialForce() {}
...
template<>
struct TStructOpsTypeTraits< FBaseRootMotionSource_RadialForce > : public TStructOpsTypeTraitsBase2< FBaseRootMotionSource_RadialForce >
{
	enum
	{
		WithNetSerializer = true,
...
USTRUCT()
struct FBaseRootMotionSource_MoveToForce : public FBaseRootMotionSource
{
	GENERATED_USTRUCT_BODY()

	FBaseRootMotionSource_MoveToForce();

	virtual ~FBaseRootMotionSource_MoveToForce() {}
...
template<>
struct TStructOpsTypeTraits< FBaseRootMotionSource_MoveToForce > : public TStructOpsTypeTraitsBase2< FBaseRootMotionSource_MoveToForce >
{
	enum
	{
		WithNetSerializer = true,
 ...
 */
USTRUCT()
struct FBaseRootMotionSource_MoveToDynamicForce : public FBaseRootMotionSource
{
	GENERATED_USTRUCT_BODY()

	FBaseRootMotionSource_MoveToDynamicForce();

	virtual ~FBaseRootMotionSource_MoveToDynamicForce() {}
...
template<>
struct TStructOpsTypeTraits< FBaseRootMotionSource_MoveToDynamicForce > : public TStructOpsTypeTraitsBase2< FBaseRootMotionSource_MoveToDynamicForce >
{
	enum
	{
		WithNetSerializer = true,
...
USTRUCT()
struct FBaseRootMotionSource_JumpForce : public FBaseRootMotionSource
{
	GENERATED_USTRUCT_BODY()

	FBaseRootMotionSource_JumpForce();

	virtual ~FBaseRootMotionSource_JumpForce() {}
...
template<>
struct TStructOpsTypeTraits< FBaseRootMotionSource_JumpForce > : public TStructOpsTypeTraitsBase2< FBaseRootMotionSource_JumpForce >
{
	enum
	{
		WithNetSerializer = true,
...
struct FBaseRootMotionSourceGroup
{
	...
	virtual ~FBaseRootMotionSourceGroup() {}

	/** Root Motion Sources currently applied in this Group */
	TArray< TSharedPtr<FBaseRootMotionSource> > RootMotionSources;

	/** Root Motion Sources to be added next frame */
	TArray< TSharedPtr<FBaseRootMotionSource> > PendingAddRootMotionSources;
	...
	UPROPERTY()
	FBaseRootMotionSourceSettings LastAccumulatedSettings;
	...
	void CleanUpInvalidRootMotion(float DeltaTime, const ABaseCharacter& Character, UBaseCharacterMovementComponent& MoveComponent);
	 ...
	 */
	void PrepareRootMotion(float DeltaTime, const ABaseCharacter& Character, const UBaseCharacterMovementComponent& InMoveComponent, bool bForcePrepareAll = false);

	/**  Helper function for accumulating override velocity into InOutVelocity */
	void AccumulateOverrideRootMotionVelocity(float DeltaTime, const ABaseCharacter& Character, const UBaseCharacterMovementComponent& MoveComponent, FVector& InOutVelocity) const;

	/**  Helper function for accumulating additive velocity into InOutVelocity */
	void AccumulateAdditiveRootMotionVelocity(float DeltaTime, const ABaseCharacter& Character, const UBaseCharacterMovementComponent& MoveComponent, FVector& InOutVelocity) const;

	/** Get rotation output of current override root motion source, returns true if OutRotation was filled */
	bool GetOverrideRootMotionRotation(float DeltaTime, const ABaseCharacter& Character, const UBaseCharacterMovementComponent& MoveComponent, FQuat& OutRotation) const;
	 ...
	 **/
	void UpdateStateFrom(const FBaseRootMotionSourceGroup& GroupToTakeStateFrom, bool bMarkForSimulatedCatchup = false);
	...
	FBaseRootMotionSourceGroup& operator=(const FBaseRootMotionSourceGroup& Other);

	/** Comparison operator - needs matching Sources along with identical states in those sources */
	bool operator==(const FBaseRootMotionSourceGroup& Other) const;
...
protected:
	...
	void AccumulateRootMotionVelocity(EBaseRootMotionAccumulateMode RootMotionType, float DeltaTime, const ABaseCharacter& Character, const UBaseCharacterMovementComponent& MoveComponent, FVector& InOutVelocity) const;

	/** Accumulates contributions for velocity into InOutVelocity for a given type of root motion from this group */
	void AccumulateRootMotionVelocityFromSource(const FBaseRootMotionSource& RootMotionSource, float DeltaTime, const ABaseCharacter& Character, const UBaseCharacterMovementComponent& MoveComponent, FVector& InOutVelocity) const;

	/** Helper function for serializing array of root motion sources */
	static void NetSerializeRMSArray(FArchive& Ar, class UPackageMap* Map, bool& bOutSuccess, TArray< TSharedPtr<FBaseRootMotionSource> >& RootMotionSourceArray, uint8 MaxNumRootMotionSourcesToSerialize = MAX_uint8);

};

template<>
struct TStructOpsTypeTraits<FBaseRootMotionSourceGroup> : public TStructOpsTypeTraitsBase2<FBaseRootMotionSourceGroup>
{
	enum
	{
		WithCopy = true,		// Necessary so that TSharedPtr<FBaseRootMotionSource> Data is copied around
		WithNetSerializer = true,
