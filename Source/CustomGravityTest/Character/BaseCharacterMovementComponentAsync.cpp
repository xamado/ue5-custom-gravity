...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BaseCharacterMovementComponentAsync)

void FBaseCharacterMovementComponentAsyncInput::Simulate(const float DeltaSeconds, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::ControlledCharacterMove(const float DeltaSeconds, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	{
...
void FBaseCharacterMovementComponentAsyncInput::PerformMovement(float DeltaSeconds, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::MaybeUpdateBasedMovement(float DeltaSeconds, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::UpdateBasedMovement(float DeltaSeconds, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
/*	if (!HasValidData())
	{
		return;
...
void FBaseCharacterMovementComponentAsyncInput::StartNewPhysics(float deltaTime, int32 Iterations, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	if ((deltaTime < UBaseCharacterMovementComponent::MIN_TICK_TIME) || (Iterations >= MaxSimulationIterations) || !bHasValidData)
	{
		return;
	}
	...
	{
		//UE_LOG(LogCharacterMovement, Log, TEXT("UBaseCharacterMovementComponent::StartNewPhysics: UpdateComponent (%s) is simulating physics - aborting."), UpdatedComponent->GetPathName());
		return;
	}
...
void FBaseCharacterMovementComponentAsyncInput::PhysWalking(float deltaTime, int32 Iterations, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	//SCOPE_CYCLE_COUNTER(STAT_CharPhysWalking);

	const FBaseCharacterMovementComponentAsyncInput& Input = *this; // TODO Refactor
	...
	// Perform the move
	while ((remainingTime >= UBaseCharacterMovementComponent::MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) /*&& CharacterOwner*/ && (/*CharacterOwner->Controller ||*/ bRunPhysicsWithNoController
		|| RootMotion.bHasAnimRootMotion || RootMotion.bHasOverrideRootMotion || (true/*LocalRole == ROLE_SimulatedProxy TODO NetRole*/)))
	{
		...
		const FBaseFindFloorResult OldFloor = Output.CurrentFloor;
		...
		else
		{
			...
			else if (Output.CurrentFloor.HitResult.bStartPenetrating && remainingTime <= 0.f)
			{
				...
				Hit.TraceEnd = Hit.TraceStart + FVector(0.f, 0.f, UBaseCharacterMovementComponent::MAX_FLOOR_DIST);
		...
		if (IsMovingOnGround(Output))
		{
			...
			if (!Output.bJustTeleported && !RootMotion.bHasAnimRootMotion && !RootMotion.bHasOverrideRootMotion && timeTick >= UBaseCharacterMovementComponent::MIN_TICK_TIME)
			{
...
void FBaseCharacterMovementComponentAsyncInput::PhysFalling(float deltaTime, int32 Iterations, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
//	SCOPE_CYCLE_COUNTER(STAT_CharPhysFalling);

	const float MIN_TICK_TIME = UBaseCharacterMovementComponent::MIN_TICK_TIME;
	...
	while ((remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations))
	{
		...
		if (BaseCharacterMovementCVars::ForceJumpPeakSubstep && OldVelocity.Z > 0.f && Velocity.Z <= 0.f && Output.NumJumpApexAttempts < MaxJumpApexAttemptsPerSimulation)
		{
...
void FBaseCharacterMovementComponentAsyncInput::PhysicsRotation(float DeltaTime, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FBaseStepDownResult* OutStepDownResult, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	if (!Output.CurrentFloor.IsWalkableFloor())
	{
		return;
...
FVector FBaseCharacterMovementComponentAsyncInput::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::CanCrouchInCurrentState(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	if (!bCanEverCrouch)
	{
		return false;
...
FVector FBaseCharacterMovementComponentAsyncInput::ConstrainInputAcceleration(FVector InputAcceleration, const FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::ScaleInputAcceleration(FVector InputAcceleration, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	return MaxAcceleration * InputAcceleration.GetClampedToMaxSize(1.0f);
}

float FBaseCharacterMovementComponentAsyncInput::ComputeAnalogInputModifier(FVector Acceleration) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::ConstrainDirectionToPlane(FVector Direction) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::ConstrainNormalToPlane(FVector Normal) const
{
	if (bConstrainToPlane)
	{
...
FVector FBaseCharacterMovementComponentAsyncInput::ConstrainLocationToPlane(FVector Location) const
{
	if (bConstrainToPlane)
	{
...
void FBaseCharacterMovementComponentAsyncInput::MaintainHorizontalGroundVelocity(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::MoveUpdatedComponent(const FVector& Delta, const FQuat& NewRotation, bool bSweep, FBaseCharacterMovementComponentAsyncOutput& Output, FHitResult* OutHitResult, ETeleportType TeleportType) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::SafeMoveUpdatedComponent(const FVector& Delta, const FQuat& NewRotation, bool bSweep, FHitResult& OutHit, FBaseCharacterMovementComponentAsyncOutput& Output, ETeleportType Teleport) const
{
	...
	{
		static IConsoleVariable* CVarMoveIgnoreFirstBlockingOverlap = IConsoleManager::Get().FindConsoleVariable(TEXT("p.MoveIgnoreFirstBlockingOverlap"));
		...
		EMoveComponentFlags& MoveComponentFlags = Output.MoveComponentFlags;
		TGuardValue<EMoveComponentFlags> ScopedFlagRestore(MoveComponentFlags, CVarMoveIgnoreFirstBlockingOverlap->GetInt() ? MoveComponentFlags : (MoveComponentFlags | IncludeBlockingOverlapsWithoutEvents));
...
void FBaseCharacterMovementComponentAsyncInput::ApplyAccumulatedForces(float DeltaSeconds, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::ClearAccumulatedForces(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	Output.PendingImpulseToApply = FVector::ZeroVector;
	Output.PendingForceToApply = FVector::ZeroVector;
...
void FBaseCharacterMovementComponentAsyncInput::SetMovementMode(EMovementMode NewMovementMode, FBaseCharacterMovementComponentAsyncOutput& Output, uint8 NewCustomMode) const
{
...
void FBaseCharacterMovementComponentAsyncInput::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	if (!bHasValidData)
	{
		return;
...
void FBaseCharacterMovementComponentAsyncInput::FindFloor(const FVector& CapsuleLocation, FBaseFindFloorResult& OutFloorResult, bool bCanUseCachedLocation, FBaseCharacterMovementComponentAsyncOutput& Output, const FHitResult* DownwardSweepResult) const
{
...
//	UE_LOG(LogCharacterMovement, VeryVerbose, TEXT("[Role:%d] FindFloor: %s at location %s"), (int32)CharacterOwner->GetLocalRole(), *GetNameSafe(CharacterOwner), *CapsuleLocation.ToString());
	...
	const float MaxFloorDist = UBaseCharacterMovementComponent::MAX_FLOOR_DIST;
	const float MinFloorDist = UBaseCharacterMovementComponent::MIN_FLOOR_DIST;
...
void FBaseCharacterMovementComponentAsyncInput::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FBaseFindFloorResult& OutFloorResult, float SweepRadius, FBaseCharacterMovementComponentAsyncOutput& Output, const FHitResult* DownwardSweepResult) const
{
	...
	if (!bSkipSweep && SweepDistance > 0.f && SweepRadius > 0.f)
	{
		...
		if (bBlockingHit)
		{
			...
			if (Hit.bStartPenetrating || !IsWithinEdgeTolerance(CapsuleLocation, Hit.ImpactPoint, CapsuleShape.Capsule.Radius))
			{
				...
				CapsuleShape.Capsule.Radius = FMath::Max(0.f, CapsuleShape.Capsule.Radius - UBaseCharacterMovementComponent::SWEEP_EDGE_REJECT_DISTANCE - UE_KINDA_SMALL_NUMBER);
			...
			const float MaxPenetrationAdjust = FMath::Max(UBaseCharacterMovementComponent::MAX_FLOOR_DIST, PawnRadius);
...
bool FBaseCharacterMovementComponentAsyncInput::FloorSweepTest(FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel, const FCollisionShape& CollisionShape, const FCollisionQueryParams& Params, const FCollisionResponseParams& ResponseParam, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	bool bBlockingHit = false;
...
bool FBaseCharacterMovementComponentAsyncInput::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const float DistFromCenterSq = (TestImpactPoint - CapsuleLocation).SizeSquared2D();
	const float ReducedRadiusSq = FMath::Square(FMath::Max(UBaseCharacterMovementComponent::SWEEP_EDGE_REJECT_DISTANCE + UE_KINDA_SMALL_NUMBER, CapsuleRadius - UBaseCharacterMovementComponent::SWEEP_EDGE_REJECT_DISTANCE));
	return DistFromCenterSq < ReducedRadiusSq;
}

bool FBaseCharacterMovementComponentAsyncInput::IsWalkable(const FHitResult& Hit) const
{
...
void FBaseCharacterMovementComponentAsyncInput::UpdateCharacterStateAfterMovement(float DeltaSeconds, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
float FBaseCharacterMovementComponentAsyncInput::GetSimulationTimeStep(float RemainingTime, int32 Iterations) const
{
...
/*#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	...
	return FMath::Max(UBaseCharacterMovementComponent::MIN_TICK_TIME, RemainingTime);
}

void FBaseCharacterMovementComponentAsyncInput::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	// Do not update velocity when using root motion or when SimulatedProxy and not simulating root motion - SimulatedProxy are repped their Velocity
	if (!bHasValidData || RootMotion.bHasAnimRootMotion || DeltaTime < UBaseCharacterMovementComponent::MIN_TICK_TIME
...
bool FBaseCharacterMovementComponentAsyncInput::ApplyRequestedMove(float DeltaTime, float MaxAccel, float MaxSpeed, float Friction, float BrakingDeceleration, FVector& OutAcceleration, float& OutRequestedSpeed, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::ShouldComputeAccelerationToReachRequestedVelocity(const float RequestedSpeed, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
float FBaseCharacterMovementComponentAsyncInput::GetMinAnalogSpeed(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	switch (Output.MovementMode)
	{
	case MOVE_Walking:
...
float FBaseCharacterMovementComponentAsyncInput::GetMaxBrakingDeceleration(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	switch (Output.MovementMode)
	{
	case MOVE_Walking:
...
void FBaseCharacterMovementComponentAsyncInput::ApplyVelocityBraking(float DeltaTime, float Friction, float BrakingDeceleration, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	FVector& Velocity = Output.Velocity;

	if (Velocity.IsZero() || !bHasValidData || RootMotion.bHasAnimRootMotion || DeltaTime < UBaseCharacterMovementComponent::MIN_TICK_TIME)
	{
		return;
	}
	...
	while (RemainingTime >= UBaseCharacterMovementComponent::MIN_TICK_TIME)
	{
	...
	if (VSizeSq <= UE_KINDA_SMALL_NUMBER || (!bZeroBraking && VSizeSq <= FMath::Square(UBaseCharacterMovementComponent::BRAKE_TO_STOP_VELOCITY)))
	{
		Velocity = FVector::ZeroVector;
	}
}

FVector FBaseCharacterMovementComponentAsyncInput::GetPenetrationAdjustment(FHitResult& HitResult) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::ResolvePenetration(const FVector& ProposedAdjustment, const FHitResult& Hit, const FQuat& NewRotation, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	...
	//	QUICK_SCOPE_CYCLE_COUNTER(STAT_MovementComponent_ResolvePenetration);
		...
		if (!bEncroached)
		{
			...
			// This line is from UBaseCharacterMovementComponent::ResolvePenetrationImpl
...
FVector FBaseCharacterMovementComponentAsyncInput::MoveComponent_GetPenetrationAdjustment(FHitResult& Hit) const
{
	if (!Hit.bStartPenetrating)
	{
	...
	static IConsoleVariable* CVarPenetrationPullbackDistance = IConsoleManager::Get().FindConsoleVariable(TEXT("p.PenetrationPullbackDistance"));

	FVector Result;
	const float PullBackDistance = FMath::Abs(CVarPenetrationPullbackDistance->GetFloat());
...
float FBaseCharacterMovementComponentAsyncInput::MoveComponent_SlideAlongSurface(const FVector& Delta, float Time, const FVector& Normal, FHitResult& Hit, FBaseCharacterMovementComponentAsyncOutput& Output, bool bHandleImpact) const
{
	if (!Hit.bBlockingHit)
	{
		return 0.f;
...
FVector FBaseCharacterMovementComponentAsyncInput::MoveComponent_ComputeSlideVector(const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	if (!bConstrainToPlane)
	{
...
bool FBaseUpdatedComponentAsyncInput::MoveComponent(const FVector& Delta, const FQuat& NewRotationQuat, bool bSweep, FHitResult* OutHit,  EMoveComponentFlags MoveFlags, ETeleportType Teleport,  const FBaseCharacterMovementComponentAsyncInput& Input, FBaseCharacterMovementComponentAsyncOutput& Output) const 
{
...
/*#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
			...
			if (Hits.Num() > 0)
			{
				...
				{
					FBaseUpdatedComponentAsyncInput::PullBackHit(Hits[HitIdx], TraceStart, TraceEnd, DeltaSize);
				}
			}
			...
			if (bHadBlockingHit || (bGatherOverlaps))
			{
				...
				for (int32 HitIdx = 0; HitIdx < Hits.Num(); HitIdx++)
				{
					...
					if (TestHit.bBlockingHit)
					{
						...
						if (!FBaseUpdatedComponentAsyncInput::ShouldIgnoreHitResult(Input.World, TestHit, Delta, nullptr/*Actor*/, MoveFlags))
						{
					...
					else if (bGatherOverlaps)//GetGenerateOverlapEvents() || bForceGatherOverlaps)
					{
						...
						{
							if (!FBaseUpdatedComponentAsyncInput::ShouldIgnoreOverlapResult(Input.World, nullptr/*Actor*/, *UpdatedComponent, TestHit.GetActor(), *OverlapComponent /*bCheckOverlapFlags=!bForceGatherOverlaps*/))
							{
...
bool FBaseUpdatedComponentAsyncInput::AreSymmetricRotations(const FQuat& A, const FQuat& B, const FVector& Scale3D) const
{
...
void FBaseUpdatedComponentAsyncInput::PullBackHit(FHitResult& Hit, const FVector& Start, const FVector& End, const float Dist)
{
...
bool FBaseUpdatedComponentAsyncInput::ShouldCheckOverlapFlagToQueueOverlaps(const UPrimitiveComponent& ThisComponent)
{
...
bool FBaseUpdatedComponentAsyncInput::ShouldIgnoreHitResult(const UWorld* InWorld, FHitResult const& TestHit, FVector const& MovementDirDenormalized, const AActor* MovingActor, EMoveComponentFlags MoveFlags)
{
	if (TestHit.bBlockingHit)
	{
		...
		static IConsoleVariable* CVarHitDistanceTolerance = IConsoleManager::Get().FindConsoleVariable(TEXT("p.HitDistanceTolerance"));
		static IConsoleVariable* CVarInitialOverlapTolerance = IConsoleManager::Get().FindConsoleVariable(TEXT("p.InitialOverlapTolerance"));
...
bool FBaseUpdatedComponentAsyncInput::ShouldIgnoreOverlapResult(const UWorld* World, const AActor* ThisActor, const UPrimitiveComponent& ThisComponent, const AActor* OtherActor, const UPrimitiveComponent& OtherComponent)
{
...
void FBaseUpdatedComponentAsyncInput::SetPosition(const FVector& InPosition) const
{
...
FVector FBaseUpdatedComponentAsyncInput::GetPosition() const
{
...
void FBaseUpdatedComponentAsyncInput::SetRotation(const FQuat& InRotation) const
{
	if (PhysicsHandle->GetPhysicsThreadAPI() == nullptr)
	{
		return;
...
FQuat FBaseUpdatedComponentAsyncInput::GetRotation() const
{
	if (PhysicsHandle && PhysicsHandle->GetPhysicsThreadAPI())
	{
...
void FBaseCharacterMovementComponentAsyncInput::HandleImpact(const FHitResult& Impact, FBaseCharacterMovementComponentAsyncOutput& Output, float TimeSlice, const FVector& MoveDelta) const
{
...
float FBaseCharacterMovementComponentAsyncInput::SlideAlongSurface(const FVector& Delta, float Time, const FVector& InNormal, FHitResult& Hit, bool bHandleImpact, FBaseCharacterMovementComponentAsyncOutput& Output) const 
{
	if (!Hit.bBlockingHit)
	{
		return 0.f;
	...
	if (IsMovingOnGround(Output))
	{
		...
		else if (Normal.Z < -UE_KINDA_SMALL_NUMBER)
		{
			...
			if (Output.CurrentFloor.FloorDist < UBaseCharacterMovementComponent::MIN_FLOOR_DIST && Output.CurrentFloor.bBlockingHit)
			{
...
FVector FBaseCharacterMovementComponentAsyncInput::ComputeSlideVector(const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::HandleSlopeBoosting(const FVector& SlideResult, const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::OnCharacterStuckInGeometry(const FHitResult* Hit, FBaseCharacterMovementComponentAsyncOutput& Output) const 
{
...
bool FBaseCharacterMovementComponentAsyncInput::CanStepUp(const FHitResult& Hit, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult& InHit, FBaseCharacterMovementComponentAsyncOutput& Output, FBaseStepDownResult* OutStepDownResult) const
{
//	SCOPE_CYCLE_COUNTER(STAT_CharStepUp);
	...
	FBaseFindFloorResult& CurrentFloor = Output.CurrentFloor;
//	float MaxStepHeight = MaxStepHeight;
	...
	if (IsMovingOnGround(Output) && CurrentFloor.IsWalkableFloor())
	{
		...
		StepTravelDownHeight = (MaxStepHeight + UBaseCharacterMovementComponent::MAX_FLOOR_DIST * 2.f);
...
bool FBaseCharacterMovementComponentAsyncInput::CanWalkOffLedges(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
//	InitCollisionParams(CapsuleParams, ResponseParam);
	const FCollisionShape CapsuleShape = GetPawnCapsuleCollisionShape(EBaseShrinkCapsuleExtent::CGSHRINK_None, Output);
...
FVector FBaseCharacterMovementComponentAsyncInput::GetPawnCapsuleExtent(const EBaseShrinkCapsuleExtent ShrinkMode, const float CustomShrinkAmount, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	...
	switch (ShrinkMode)
	{
	case EBaseShrinkCapsuleExtent::CGSHRINK_None:
		return CapsuleExtent;

	case EBaseShrinkCapsuleExtent::CGSHRINK_RadiusCustom:
		RadiusEpsilon = CustomShrinkAmount;
		break;

	case EBaseShrinkCapsuleExtent::CGSHRINK_HeightCustom:
		HeightEpsilon = CustomShrinkAmount;
		break;

	case EBaseShrinkCapsuleExtent::CGSHRINK_AllCustom:
		RadiusEpsilon = CustomShrinkAmount;
		HeightEpsilon = CustomShrinkAmount;
		break;
	...
	default:
		...
		//UE_LOG(LogCharacterMovement, Warning, TEXT("Unknown EBaseShrinkCapsuleExtent in UBaseCharacterMovementComponent::GetCapsuleExtent"));
		break;
	}
...
FCollisionShape FBaseCharacterMovementComponentAsyncInput::GetPawnCapsuleCollisionShape(const EBaseShrinkCapsuleExtent ShrinkMode, FBaseCharacterMovementComponentAsyncOutput& Output, const float CustomShrinkAmount) const
{
	FVector Extent = GetPawnCapsuleExtent(ShrinkMode, CustomShrinkAmount, Output);
	return FCollisionShape::MakeCapsule(Extent);
}

void FBaseCharacterMovementComponentAsyncInput::TwoWallAdjust(FVector& OutDelta, const FHitResult& Hit, const FVector& OldHitNormal, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::RevertMove(const FVector& OldLocation, UPrimitiveComponent* OldBase, const FVector& PreviousBaseLocation, const FBaseFindFloorResult& OldFloor, bool bFailMove, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
ETeleportType FBaseCharacterMovementComponentAsyncInput::GetTeleportType(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::HandleWalkingOffLedge(const FVector& PreviousFloorImpactNormal, const FVector& PreviousFloorContactNormal, const FVector& PreviousLocation, float TimeDelta) const
{
	ensure(false); // TODO HandleWalkingOffLedge
}

bool FBaseCharacterMovementComponentAsyncInput::ShouldCatchAir(const FBaseFindFloorResult& OldFloor, const FBaseFindFloorResult& NewFloor) const
{
	return false;
}

void FBaseCharacterMovementComponentAsyncInput::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	// start falling 
	const float DesiredDist = Delta.Size();
...
void FBaseCharacterMovementComponentAsyncInput::SetBaseFromFloor(const FBaseFindFloorResult& FloorResult, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::AdjustFloorHeight(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	...
	{
		if (OldFloorDist < UBaseCharacterMovementComponent::MIN_FLOOR_DIST && CurrentFloor.LineDist >= UBaseCharacterMovementComponent::MIN_FLOOR_DIST)
		{
	...
	if (OldFloorDist < UBaseCharacterMovementComponent::MIN_FLOOR_DIST || OldFloorDist > UBaseCharacterMovementComponent::MAX_FLOOR_DIST)
	{
		FHitResult AdjustHit(1.f);
		const float InitialZ = UpdatedComponentInput->GetPosition().Z;
		const float AvgFloorDist = (UBaseCharacterMovementComponent::MIN_FLOOR_DIST + UBaseCharacterMovementComponent::MAX_FLOOR_DIST) * 0.5f;
...
bool FBaseCharacterMovementComponentAsyncInput::ShouldComputePerchResult(const FHitResult& InHit, FBaseCharacterMovementComponentAsyncOutput& Output, bool bCheckRadius) const
{
	...
	if (GetPerchRadiusThreshold() <= UBaseCharacterMovementComponent::SWEEP_EDGE_REJECT_DISTANCE)
	{
		return false;
	}
...
bool FBaseCharacterMovementComponentAsyncInput::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FBaseFindFloorResult& OutPerchFloorResult, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
float FBaseCharacterMovementComponentAsyncInput::GetPerchRadiusThreshold() const
{
	// Don't allow negative values.
	return FMath::Max(0.f, PerchRadiusThreshold);
}

float FBaseCharacterMovementComponentAsyncInput::GetValidPerchRadius(const FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	const float PawnRadius = Output.ScaledCapsuleRadius;
	return FMath::Clamp(PawnRadius - GetPerchRadiusThreshold(), 0.11f, PawnRadius);
}

bool FBaseCharacterMovementComponentAsyncInput::CheckFall(const FBaseFindFloorResult& OldFloor, const FHitResult& Hit, const FVector& Delta, const FVector& OldLocation, float remainingTime, float timeTick, int32 Iterations, bool bMustJump, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	if (!bHasValidData)
	{
		return false;
...
FVector FBaseCharacterMovementComponentAsyncInput::GetFallingLateralAcceleration(float DeltaTime, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
float FBaseCharacterMovementComponentAsyncInput::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::ShouldLimitAirControl(float DeltaTime, const FVector& FallAcceleration, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	return (FallAcceleration.SizeSquared2D() > 0.f);
}

FVector FBaseCharacterMovementComponentAsyncInput::LimitAirControl(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, bool bCheckForValidLandingSpot, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
void FBaseCharacterMovementComponentAsyncInput::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::SetPostLandedPhysics(const FHitResult& Hit, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	// TODO Other movement modes
...
void FBaseCharacterMovementComponentAsyncInput::SetDefaultMovementMode(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::ShouldRemainVertical(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	// Always remain vertical when walking or falling.
	return IsMovingOnGround(Output) || IsFalling(Output);
}

bool FBaseCharacterMovementComponentAsyncInput::CanAttemptJump(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::DoJump(bool bReplayingMoves, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
bool FBaseCharacterMovementComponentAsyncInput::IsJumpAllowed() const
{
...
* 
*/
float FBaseCharacterMovementComponentAsyncInput::GetMaxSpeed(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	switch (Output.MovementMode)
	{
	case MOVE_Walking:
...
bool FBaseCharacterMovementComponentAsyncInput::IsCrouching(const FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	return Output.bIsCrouched;
}

bool FBaseCharacterMovementComponentAsyncInput::IsFalling(const FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	return (Output.MovementMode == MOVE_Falling);// && UpdatedComponent;
}

bool FBaseCharacterMovementComponentAsyncInput::IsFlying(const FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	return (Output.MovementMode == MOVE_Flying);// && UpdatedComponent;
}

bool FBaseCharacterMovementComponentAsyncInput::IsMovingOnGround(const FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	return ((Output.MovementMode == MOVE_Walking) || (Output.MovementMode == MOVE_NavWalking));//&& UpdatedComponent;
}

bool FBaseCharacterMovementComponentAsyncInput::IsExceedingMaxSpeed(float MaxSpeed, const FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
FRotator FBaseCharacterMovementComponentAsyncInput::ComputeOrientToMovementRotation(const FRotator& CurrentRotation, float DeltaTime, FRotator& DeltaRotation, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::RestorePreAdditiveRootMotionVelocity(FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterMovementComponentAsyncInput::ApplyRootMotionToVelocity(float deltaTime, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::ConstrainAnimRootMotionVelocity(const FVector& RootMotionVelocity, const FVector& CurrentVelocity, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaSeconds, const FVector& CurrentVelocity) const
{
...
FVector FBaseCharacterMovementComponentAsyncInput::GetAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	// Boost
...
void FBaseCharacterAsyncInput::FaceRotation(FRotator NewControlRotation, float DeltaTime, const FBaseCharacterMovementComponentAsyncInput& Input, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterAsyncInput::CheckJumpInput(float DeltaSeconds, const FBaseCharacterMovementComponentAsyncInput& Input, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterAsyncInput::ClearJumpInput(float DeltaSeconds, const FBaseCharacterMovementComponentAsyncInput& Input, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
	if (Output.CharacterOutput->bPressedJump)
	{
...
bool FBaseCharacterAsyncInput::CanJump(const FBaseCharacterMovementComponentAsyncInput& Input, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterAsyncInput::ResetJumpState(const FBaseCharacterMovementComponentAsyncInput& Input, FBaseCharacterMovementComponentAsyncOutput& Output) const
{
...
void FBaseCharacterAsyncInput::OnMovementModeChanged(EMovementMode PrevMovementMode, const FBaseCharacterMovementComponentAsyncInput& Input, FBaseCharacterMovementComponentAsyncOutput& Output, uint8 PreviousCustomMode)
{
...
FName FBaseCharacterMovementComponentAsyncCallback::GetFNameForStatId() const
{
	const static FLazyName StaticName("FBaseCharacterMovementComponentAsyncCallback");
	return StaticName;
}

void FBaseCharacterMovementComponentAsyncCallback::OnPreSimulate_Internal()
{
	BasePreSimulateImpl<FBaseCharacterMovementComponentAsyncInput, FBaseCharacterMovementComponentAsyncOutput>(*this);
}

void FBaseCharacterMovementComponentAsyncOutput::Copy(const FBaseCharacterMovementComponentAsyncOutput& Value)
{
...
FRotator FBaseCharacterMovementComponentAsyncOutput::GetDeltaRotation(const FRotator& InRotationRate, float InDeltaTime)
{
...
float FBaseCharacterMovementComponentAsyncOutput::GetAxisDeltaRotation(float InAxisRotationRate, float InDeltaTime)
{
