...
USTRUCT()
struct FBaseRepRootMotionMontage
{
	...
	UPROPERTY()
	FBaseRootMotionSourceGroup AuthoritativeRootMotion;
...
USTRUCT()
struct FBaseSimulatedRootMotionReplicatedMove
{
	GENERATED_USTRUCT_BODY()
	...
	UPROPERTY()
	FBaseRepRootMotionMontage RootMotion;
};
 ...
 *
 * @see APawn, UBaseCharacterMovementComponent
...
private:
	...
	UPROPERTY(Category=Character, VisibleAnywhere, BlueprintReadOnly, meta=(AllowPrivateAccess = "true"))
	TObjectPtr<UBaseCharacterMovementComponent> CharacterMovement;
...
public:
	...
	void ServerMovePacked(const FBaseCharacterServerMovePackedBits& PackedBits);
	void ServerMovePacked_Implementation(const FBaseCharacterServerMovePackedBits& PackedBits);
	bool ServerMovePacked_Validate(const FBaseCharacterServerMovePackedBits& PackedBits);
	...
	void ClientMoveResponsePacked(const FBaseCharacterMoveResponsePackedBits& PackedBits);
	void ClientMoveResponsePacked_Implementation(const FBaseCharacterMoveResponsePackedBits& PackedBits);
	bool ClientMoveResponsePacked_Validate(const FBaseCharacterMoveResponsePackedBits& PackedBits);
	...
	void ClientAdjustRootMotionSourcePosition_Implementation(float TimeStamp, FBaseRootMotionSourceGroup ServerRootMotion, bool bHasAnimRootMotion, float ServerMontageTrackPosition, FVector ServerLoc, FVector_NetQuantizeNormal ServerRotation, float ServerVelZ, UPrimitiveComponent* ServerBase, FName ServerBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode);
...
public:
	...
	FORCEINLINE UBaseCharacterMovementComponent* GetCharacterMovement() const { return CharacterMovement; }
...
protected:
	...
	UPROPERTY()
	struct FBaseBasedMovementInfo BasedMovement;
	...
	struct FBaseBasedMovementInfo ReplicatedBasedMovement;
...
public:
	...
	FORCEINLINE const FBaseBasedMovementInfo& GetBasedMovement() const { return BasedMovement; }
	
	/** Accessor for ReplicatedBasedMovement */
	FORCEINLINE const FBaseBasedMovementInfo& GetReplicatedBasedMovement() const { return ReplicatedBasedMovement; }
...
public:
	...
	UPROPERTY(Transient)
	TArray<FBaseSimulatedRootMotionReplicatedMove> RootMotionRepMoves;
	...
	bool CanUseRootMotionRepMove(const FBaseSimulatedRootMotionReplicatedMove& RootMotionRepMove, const FAnimMontageInstance& ClientMontageInstance) const;

	/** Restore actor to an old buffered move. */
	bool RestoreReplicatedMove(const FBaseSimulatedRootMotionReplicatedMove& RootMotionRepMove);
	...
	struct FBaseRepRootMotionMontage RepRootMotion;
